'''
To extract the relevance scores of all sentences in the relevant documents, 
and couple them with the sentences, to feed as input to 
the abstractive summarisation model.
'''
...
def CreateWindows(indices,sentences):
	...
	for i in indices:
		...
		for x in range(max(0,i-window),min((i+window+1),len(sentences))):
			...
			temp += sentences[x]
			temp += "\n"
		AllSentRelPairs.append(temp)
	return AllSentRelPairs
...
def create_mapping(indices, scores, sentences):
	goodSentRel = []
	for i in range(0, len(indices)):
		sent = sentences[indices[i]]
		rel = scores[i]
		rel_vec = rel*np.ones(len(sent))
		goodSentRel.append([sent, rel_vec])
	return goodSentRel
...
def ExtrSen(doc,query,method):
	...
	if method == 2:
		AllSentRelPairs = VSmethod(clean_sentences,clean_query,word_idf,sentences)
	elif method == 3:
		AllSentRelPairs = Tfidfmethod(clean_sentences,clean_query,word_idf,sentences)
	elif method == 4:
		AllSentRelPairs = LuhnClusters(sentences,word_cf,clean_query)
	else:
		AllSentRelPairs = QueryBiasedLSA(clean_sentences,word_idf,clean_query,sentences)
...
def VSmethod(clean_sentences,clean_query,word_idf,sentences):
	...
	AllSentRelPairs  =create_mapping(GoodSent, scores, sentences)
	# AllSentRelPairs = CreateWindows(GoodSent,sentences)
	return AllSentRelPairs
...
def LuhnClusters(sentences,word_cf,clean_query):
	...
	#normalizing the scores
	OverallScores = normalize(OverallScores[:,np.newaxis], axis=0).ravel()
	ind = np.argsort(-OverallScores)
	final_rel_scores = []
	# print OverallScores
	# print ind
	# for i in ind[:Cutoff]:
	for i in range(0, len(OverallScores)):
		GoodSent.append(i)
		final_rel_scores.append(OverallScores[i])
	AllSentRelPairs  =create_mapping(GoodSent, final_rel_scores, sentences)	
	# AllSentRelPairs = CreateWindows(GoodSent,sentences)
	return AllSentRelPairs
...
def extractSent(directory, name_list, query, method):
	...
	for filename in input_files:
		...
		with open(filename, 'r') as f:
			AllSentRelPairs = ExtrSen(f.read(), query, method)
		allDocs.append(AllSentRelPairs)
		# for i, para in enumerate(AllSentRelPairs):
