...
def init_config():
    ...
    parser.add_argument('--max_epochs', default=2, type=int, help='maximum number of training epochs')
...
class NMT(nn.Module):
    ...
    def decode_sample(self, src_encoding, dec_init_vec, tgt_sents):
        ...
        for i in range(tgt_len):
            ...
            ctx_t, alpha_t, temporal_att = self.dot_prod_temporal_attention(h_t, temporal_att, src_encoding, src_encoding_att_linear)
    ...
    def dot_prod_temporal_attention(self, h_t, temporal_att, src_encoding, src_encoding_att_linear, mask=None):
        """
...
def RL_train(args):
    ...
    while True:
        ...
        for src_sents, tgt_sents in data_iter(train_data, batch_size = args.batch_size):
            ...
            debug_ = cross_entropy_loss( scores.view(-1, scores.size(2)), sample_y.view(-1) )
            # print debug_
            word_loss = debug_.view(scores.size(0), scores.size(1),1)
